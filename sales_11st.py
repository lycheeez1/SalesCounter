...
def cal_11st(path):
    ...
    df['상품명N'] = df['상품명'].str.replace(pat='[a-z]{2}[0-9]{3}', repl=r'', regex=True)
    df['상품명N'] = df['상품명N'].str.replace(pat='[+][스][너][글]|[+][증][정]\d[.]\d[L]|[1][0][T]', repl=r'', regex=True)
    df['옵션N'] = df['주문선택사항'].str.replace(pat=r' ', repl=r'', regex=True)
    df['옵션N'] = df['옵션N'].str.replace(pat='[선]\w+[:]|[-]\d+[개]', repl=r'', regex=True)
    df['옵션N'] = df['옵션N'].str.replace(pat='[색][상][:]|[사][이][즈][:]|[발][송][일][:]', repl=r'', regex=True)
    df['옵션N'] = df['옵션N'].str.replace(pat='[종][류][:]|[라][인][:]|[사][은][품][:]', repl=r'', regex=True)
    df['옵션N'] = df['옵션N'].str.replace(pat='[(][+-]\d+[원][)]|[(]\d+[원][)]', repl=r'', regex=True)
    df['옵션N'] = df['옵션N'].str.replace(pat='[옵][션][가]\d+|[옵][션][가][-]\d+', repl=r'', regex=True)
    df['옵션N'] = df['옵션N'].str.replace(pat='[★][증][정][:][★]\w+[,]|[+][증][정]\d[.]\d[L]', repl=r'', regex=True)
    df['옵션N'] = df['옵션N'].str.replace(pat='[증][정][:]\w+[,]', repl=r'', regex=True)
...
#    df.drop(columns=['결제일','상태', '판매사이트 상품코드', '판매자상품코드', '주문선택사항금액', '판매가', '배송비금액', '배송방법(원본)', '송장번호', '수집일'], inplace=True)
    ...
    ################################################
    platform = df['판매사이트명']
    df_11st = df[platform.str.contains('11번가')]

    plist, qlist, nlist, namelist = [], [], [], []
    plist_mul, qlist_mul, nlist_mul, oplist_mul = [], [], [], []

    for pn, pc, op, n in zip(df_11st['상품명N'], df_11st['상품코드'], df_11st['옵션N'], df_11st['주문수량']):
        # 상품코드
        try:
            product_code = []
            # 종합 딜
            ...
            print("[ERROR_00_11st_t1]", pc, "is out of range")
            continue
        except KeyError as e2:
            print("[ERROR_00_11st_t1]", str(e2), "is not available")
            continue
        ...
        try:
            ...
            print("[ERROR_00_11st_t2]", pc, "is out of range")
            continue
        except KeyError as e2:
            print("[ERROR_00_11st_t2]", str(e2), "is not available")
            continue
...
#                            oplist.append(option)
                        ...
                        print("[ERROR_00_11st_if]", pcode, "is not available")
                        continue
                else:
...
#                                    print(mrate, "% || ", value, "|| ", opp)
                                ...
                                print("[ERROR_02_11st_if]", pp, "is not available")
                                continue
                    try :
...
#                            print("else=>", r[0], r[2].strip(), r[3], r[4])
                        ...
                        print("[ERROR_01_11st_t2]", str(e))
#                        print("=>", pl, opl, ql)
                        continue
            else:
#                print("[ERROR_03_11st_if]", pl[0], "is not available")
                try:
                    pl = pl[1:]
                    for pp, qq in zip(pl, ql):
                        val2 = code_table[pp]
...
#                        print("if2=>", pp, val2.strip(), qq, n)
                    ...
                    print("[ERROR_01_11st_t3]", str(e))
                    continue
        # 품번 없을 시 예외 (KeyError로 인한 Runtime에러 방지)
        except Exception as e:
            print("[ERROR_01_11st_t1]", str(e))
#            print("=>", pl, opl, ql)
#            rate_list.clear()
            continue
