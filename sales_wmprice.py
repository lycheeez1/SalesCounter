...
def cal_wmprice(path):
    ...
    #df['옵션N'] = df['옵션N'].str.replace(pat='[(]\d[.]\d\w+[)]|[(]\d[L][)]|[{]\d+', repl=r'', regex=True)
#    df['옵션N'] = df['옵션N'].str.replace(pat='[0-9]{2}[)]', repl=r'', regex=True)  # 옵션번호 필요없
#    df['상품코드'] = df['판매자상품코드']
    ...
    ################################################
    platform = df['판매사이트명']
    df_wmp = df[platform.str.contains('위메프')]

    plist, qlist, nlist, namelist = [], [], [], []
    plist_mul, qlist_mul,nlist_mul, oplist_mul = [], [], [], []

    for pn, pc, op, n in zip(df_wmp['상품명N'], df_wmp['상품코드'], df_wmp['옵션N'], df_wmp['주문수량']):
        # 상품코드
        try:
            product_code = []
            # 종합 딜
...
#                print(deal_code, option_num)
            ...
            print("[ERROR_00_wmp_t1]", pc, "is out of range")
            continue
        except KeyError as e2:
            print("[ERROR_00_wmp_t1]", str(e2), "is not available")
            continue
        ...
        try:
            if op == '-':
                if product_code[0][0] == 'd':
                    ...
                    register_quantity = re.compile('\d+[개]|[xX]\d+|\d+[캔]|[(]\d+[)]').findall(pn)   # 등록 개수 (리스트)
                x = pn.count('+')
            ...
            print("[ERROR_00_wmp_t2]", pc, "is out of range")
            continue
        except KeyError as e2:
            print("[ERROR_00_wmp_t2]", str(e2), "is not available")
            continue
            ...
            try:
                if len(pcode) == 5:
                    ...
                    if pcode in code_table.keys():
                        ...
                        print("[ERROR_00_wmp_if]", pcode, "is not available")
                        continue
                else:
    ...
    for p, q, op, n in zip(plist_mul, qlist_mul, oplist_mul, nlist_mul):
        ...
        if '자일리톨' in op and '키즈' in op:
            op = re.sub('키즈', '', op)
            op = op.replace('자일리톨', '2080 키즈 자일리톨')
        if '립톤제로' in op:
        ...
        try:
            for pp in pl:
                if pp in code_table.keys():
                    ...
                    print("[ERROR_01_wmp_for]", pp, "is not available")
                    pl.remove(pp)
...
#                                    print(mrate, "% || ", value, "|| ", opp)
                                ...
                                print("[ERROR_02_wmp_if]", pp, "is not available")
                                continue
                    try :
...
#                        print("if2=>", pp, val2.strip(), qq, n)
                    ...
                    print("[ERROR_01_wmp_t3]", str(e))
                    continue
        # 품번 없을 시 예외 (KeyError로 인한 Runtime에러 방지)
        except IndexError as e1:
            print("[ERROR_01_wmp_t1]", pl[0], "is out of range")
#            print("=>", pl, opl, ql)
#            rate_list.clear()
            continue
        except KeyError as e2:
            print("[ERROR_01_wmp_t1]", str(e2), "is not available")
#            print("=>", pl, opl, ql)
#            rate_list.clear()
            continue
